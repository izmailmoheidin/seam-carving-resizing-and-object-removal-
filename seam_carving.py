...
class SeamCarver:
    ...
    def object_removal(self):
        ...
        """
        rotate = False
        assert self.mask.shape == self.out_image.shape[:2], "Mask and image dimensions do not match!"
    ...
    def delete_seam(self, seam_idx):
        ...
        for row in range(m):
            ...
            assert np.all(seam_idx < self.out_image.shape[1]), "Seam index out of bounds!"
